...
START_PATH=${PWD}
touch ${START_PATH}/actspot.log
OUTPUTLOG=${START_PATH}/actspot.log
...
# Defaults
QH_LANG_DEFAULT="en"
QH_PORT_DEFAULT="1453"
QH_MYSQL_ROOT_PASS_DEFAULT="actspot"
QH_MYSQL_USER_NAME_DEFAULT="actspot"
QH_MYSQL_USER_PASS_DEFAULT="actspot"
QH_MYSQL_DBNAME_DEFAULT="actspot"
QH_ZONE_NAME_DEFAULT="ACTSPOT"
...
# Panel /usr/local/www içine taşınıyor...

_movepanelfilestowww
...
_selectLanguage() {
    read -p "Select your language (en/tr) [$LANG_DEFAULT]: " LANG
...
_mysqlSettings() {
    ...
    cat <<EOF > /usr/local/actspot/install/client.cnf
[client]
user = root
password = ${QH_MYSQL_ROOT_PASS}
host = localhost
EOF
    cp /usr/local/actspot/public/db_EXAMPLE.php /usr/local/actspot/public/db.php
    sed -i .bak -e "s/{QH_MYSQL_USER_NAME}/$QH_MYSQL_USER_NAME/g" /usr/local/actspot/public/db.php
    sed -i .bak -e "s/{QH_MYSQL_USER_PASS}/$QH_MYSQL_USER_PASS/g" /usr/local/actspot/public/db.php
    sed -i .bak -e "s/{QH_MYSQL_DBNAME}/$QH_MYSQL_DBNAME/g" /usr/local/actspot/public/db.php

    sed -i .bak -e "s/{QH_MYSQL_ROOT_PASS}/$QH_MYSQL_ROOT_PASS/g" /usr/local/actspot/install/qhotspot.sql
    sed -i .bak -e "s/{QH_MYSQL_USER_NAME}/$QH_MYSQL_USER_NAME/g" /usr/local/actspot/install/qhotspot.sql
    sed -i .bak -e "s/{QH_MYSQL_USER_PASS}/$QH_MYSQL_USER_PASS/g" /usr/local/actspot/install/qhotspot.sql
    sed -i .bak -e "s/{QH_MYSQL_DBNAME}/$QH_MYSQL_DBNAME/g" /usr/local/actspot/install/qhotspot.sql

    mysql --defaults-extra-file=/usr/local/actspot/install/client.cnf < /usr/local/actspot/install/qhotspot.sql
    echo ${L_OK} 1>&3

    # MySQL icin watchdog scripti olusturuluyor.
    echo -n ${L_MYSQLWATCHDOG} 1>&3
    cat <<EOF > /usr/local/bin/actspot_check.sh
#!/usr/bin/env sh
...
EOF
    chmod +x /usr/local/bin/actspot_check.sh
    echo ${L_OK} 1>&3
}
...
_qhotspotSettings() {
    ...
    cp /usr/local/actspot/install/qhotspotconfig.php /etc/phpshellsessions/qhotspotconfig
...
_movepanelfilestowww() {
    ...
    mv /usr/local/actspot/public /usr/local/www/actspot

}
_clean() {
    rm -rf ${START_PATH}/lang_*
    rm -rf /usr/local/actspot/install/client.cnf*
    rm -rf /usr/local/actspot/install/qhotspot.sql*
    rm -rf /usr/local/actspot/install/qhotspot.sh*
    rm -rf /usr/local/actspot/install/qhotspotconfig.php
}
